 ...
 * @author ccv
 */
public class UpdateComunaServlet extends HttpServlet {

    private static final Logger LOGGER = Logger.getLogger(UpdateComunaServlet.class.getName());
    ...
    protected void updateRegion(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //update region
            if (comunasSession != null && id != null) {
                /*comunas = (List<ComunaVO>) comunasSession;
                ComunaVO updateComuna = comunas.stream().filter(
                        comuna -> comuna.getId() == Short.valueOf(id)).findFirst().get();
            */
                if(regionesSession != null && idRegion != null){
                
                    /*regiones = (List<RegionVO>)regionesSession;
                    RegionVO updateRegion = regiones.stream().filter(
                        region -> region.getId() == Short.valueOf(idRegion)).findFirst().get();
                    
                    updateComuna.setRegion(new RegionVO(updateRegion));*/
                }
                
                //updateComuna.setNombre(nombre);
                //updateComuna.setDescripcion(descripcion);
            }
